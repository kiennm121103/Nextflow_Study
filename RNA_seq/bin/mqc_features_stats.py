...
def mqc_feature_stat(bfile, features, outfile, sname=None):
    """
    Extracts and summarizes feature statistics from a given binary file.

    Parameters:
    bfile (str): Path to the binary file containing feature data.
    features (str): Path to the output file where summarized statistics will be saved.
    sname (str, optional): Sample name to include in the output. Defaults to None.
    """
    # If sample name not give use file name
    if not sname:
    ...
    try:
        ...
        logger.error(f"Trouble reading the biocount file {bfile}: {e}")
        return
...
# Calculate percentage for each requested features
fpercent = {f: (fcounts[f] /total_count) * 100 if f in fcounts else 0 for f in features}
if len(fpercent) ==0:
    logger.error("Any of give features '{}' not found in the biocount file".format(", ".join(features),bfile))
    return

# Prepare the output strings
out_head, out_value, out_mpc = ("Sample", "'{}'".format(sname), mpc_main)
...
# Wwrite the outout to a file
with open(outfile, "w") as ofl:
    out_final = "\n".join([out_mpc, out_head, out_value]).strip()
    ofl.write(out_final + "\n")
